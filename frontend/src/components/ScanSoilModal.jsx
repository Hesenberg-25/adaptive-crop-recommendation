...
import { Camera, Upload, X, Loader2, Image as ImageIcon, AlertCircle, CheckCircle2 } from 'lucide-react';
import axios from 'axios';
import { useAuth } from '../context/AuthContext';

const ScanSoilModal = ({ isOpen, onClose, onScanComplete }) => {
  const { token } = useAuth();
      ...
      {isOpen && (
        ...
        >
          <motion.div 
            ...
            className="w-full max-w-lg glass-panel bg-white/90 dark:bg-[#111A0E]/95 overflow-hidden rounded-[2rem] shadow-2xl border border-slate-200 dark:border-white/10"
          >
            {/* Header */}
                ...
                <div className="flex flex-col gap-4">
                  <div className="bg-emerald-50 dark:bg-emerald-900/20 rounded-2xl p-5 border border-emerald-200 dark:border-emerald-800">
                    <h3 className="font-bold text-emerald-800 dark:text-emerald-300 mb-4 flex items-center gap-2">
                      <CheckCircle2 className="w-5 h-5" /> AI Analysis Complete
                    </h3>
                    <div className="grid grid-cols-2 gap-3">
                      <div className="flex flex-col bg-white dark:bg-black/40 p-3 rounded-xl shadow-sm">
                      ...
                      <div className="flex flex-col bg-white dark:bg-black/40 p-3 rounded-xl shadow-sm">
                        ...
                        <span className="font-bold text-slate-800 dark:text-white">{scanResult.N ?? '--'} mg/kg</span>
                      </div>
                      <div className="flex flex-col bg-white dark:bg-black/40 p-3 rounded-xl shadow-sm">
                        <span className="text-xs text-slate-500 font-medium">Phosphorus (P)</span>
                        <span className="font-bold text-slate-800 dark:text-white">{scanResult.P ?? '--'} mg/kg</span>
                      </div>
                      <div className="flex flex-col bg-white dark:bg-black/40 p-3 rounded-xl shadow-sm">
                        <span className="text-xs text-slate-500 font-medium">Potassium (K)</span>
                        <span className="font-bold text-slate-800 dark:text-white">{scanResult.K ?? '--'} mg/kg</span>
                      </div>
                      <div className="flex flex-col bg-white dark:bg-black/40 p-3 rounded-xl shadow-sm col-span-2">
                        <span className="text-xs text-slate-500 font-medium">pH Level</span>
                        <span className="font-bold text-slate-800 dark:text-white">{scanResult.pH ?? '--'}</span>
                      </div>
                    </div>
                  </div>
                  
                  <div className="flex gap-3">
                    <button onClick={() => setScanResult(null)} className="flex-1 py-3 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 font-bold rounded-xl transition-colors">
                      Discard
                    </button>
                    <button onClick={() => { onScanComplete(scanResult); onClose(); }} className="flex-1 py-3 bg-emerald-500 hover:bg-emerald-600 text-white font-bold rounded-xl shadow-lg shadow-emerald-500/30 transition-all active:scale-95">
                      Confirm & Apply
                    </button>
                  </div>
                </div>
